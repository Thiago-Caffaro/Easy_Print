...
The capability contracts cover the categories the product can safely render, missing categories, and preserved-but-non-renderable driver-specific options. Submission and active-job contracts cover neutral response shapes for a job identifier, multiple queues, and normalized pending/processing states. They do not assert that the Epson reference device or any other printer supports those features or response variants.
