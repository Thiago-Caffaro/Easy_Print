...
A reverse proxy is optional and is not included in the reference Compose topology. When one is used, it must reject request bodies and headers at the same or stricter limits before proxying. The versioned [Nginx server fragment](../deploy/nginx/easy-print-server.conf.example) applies the defaults above and enables request buffering so rejection occurs before the body reaches PHP. Its upstream matches the loopback-bound Compose default; replace it only when the local topology differs, and include the fragment inside the HTTPS `server` block. If custom environment limits are lower, lower the proxy values at the same time.
